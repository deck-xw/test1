...
void lcd_driver_clear_screen(uint16_t x_size,uint16_t y_size,uint16_t color)
{
	uint32_t *lcd_frame_buff = (uint32_t *)LCD_FRAMME_BUFF_ADDR_1;
	int i = 0,j=0;
	...
	for(j=0; j<y_size; j++)
	{
		for(i=0; i<x_size; i++)
		{
			*((uint32_t *)(lcd_frame_buff + j*x_size/2 + i)) = temp_color;
		}
	}
	lcd_driver_display_frame_with_gui(lcd_frame_buff, x_size, y_size);
}
