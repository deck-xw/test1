...
void uvc_get_param(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[2], "cur") == 0) {
		CLI_LOGI("uvc cur 0x%x!\r\n", bk_usb_uvc_attribute_op(UVC_ATTRIBUTE_GET_CUR, attribute, &param));
	} else if (os_strcmp(argv[2], "min") == 0) {
		CLI_LOGI("uvc min 0x%x!\r\n", bk_usb_uvc_attribute_op(UVC_ATTRIBUTE_GET_MIN, attribute, &param));
	} else if (os_strcmp(argv[2], "max") == 0) {
		CLI_LOGI("uvc max 0x%x!\r\n", bk_usb_uvc_attribute_op(UVC_ATTRIBUTE_GET_MAX, attribute, &param));
	} else if (os_strcmp(argv[2], "info") == 0) {
		CLI_LOGI("uvc info 0x%x!\r\n", bk_usb_uvc_attribute_op(UVC_ATTRIBUTE_GET_INFO, attribute, &param));
	} else if (os_strcmp(argv[2], "len") == 0) {
		CLI_LOGI("uvc len 0x%x!\r\n", bk_usb_uvc_attribute_op(UVC_ATTRIBUTE_GET_LEN, attribute, &param));
	} else if (os_strcmp(argv[2], "res") == 0) {
		CLI_LOGI("uvc res 0x%x!\r\n", bk_usb_uvc_attribute_op(UVC_ATTRIBUTE_GET_RES, attribute, &param));
	} else if (os_strcmp(argv[2], "def") == 0) {
		CLI_LOGI("uvc def 0x%x!\r\n", bk_usb_uvc_attribute_op(UVC_ATTRIBUTE_GET_DEF, attribute, &param));
	} else {
		cli_usb_help();
		return;
	}
...
void uvc_set_param(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	bk_usb_uvc_attribute_op(UVC_ATTRIBUTE_SET_CUR, attribute, &param);
...
void uvc_start_stream(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if (os_strcmp(argv[1], "start") == 0) {
		...
		UVC_ResolutionFramerate support_config[10] = {0};
		bk_uvc_get_resolution_framerate((void *)support_config, 10);
	} else {
		cli_usb_help();
		return;
	}
