...
static short g_blend_hor_align = 0;   //0��ˮƽ�����  1��ˮƽ����   2��ˮƽ�Ҷ���
static short g_blend_ver_offset = 0;  //��ֱƫ��
...
#ifdef DISPLAY_PIPELINE_TASK
void lcd_brash_test(frame_buffer_t *frame)
{
	uint32_t *lcd_frame_buff = (uint32_t *)(frame->frame);
	int i = 0,j=0;
	uint32_t temp_color = 0xf800;
	temp_color <<= 16;
	temp_color |= 0xf800;

    //lcd_frame_buff+=480*((816-800)/2);
	
	for(j=0; j<100; j++)
	{
		for(i=0; i<480; i++)
		{
			*((uint32_t *)(lcd_frame_buff + j*480/2 + i)) = temp_color;
		}
	}
}
