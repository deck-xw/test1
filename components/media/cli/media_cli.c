...
void media_cli_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	else
	{
		...
		if (os_strcmp(argv[1], "lcd") == 0)
		{
			...
			else  if (os_strcmp(argv[2], "brush") == 0)
			{
				extern void lcd_driver_clear_screen(uint16_t x_size,uint16_t y_size,uint16_t color);
                lcd_driver_clear_screen(170,320,strtol(argv[3],NULL,16));
			}
#if (CONFIG_LVGL)
