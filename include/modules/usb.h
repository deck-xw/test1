 ...
 *
 */
bk_err_t bk_usb_uvc_attribute_op(E_UVC_ATTRIBUTE_OP ops, E_UVC_ATTRIBUTE_T attribute, uint32_t *param);
 ...
 *    - others: other errors.
 */
bk_err_t bk_usb_uac_set_hz(USB_DEVICE_WORKING_STATUS dev, uint32 hz);
 ...
 *   - dev:
 *        enumeration USB_DEVICE_WORKING_STATUS
 *   - interfacedesc:
...
/**
 * @brief     Set UAC volume value
 *
 * This API set volume:
 *   - One parameters param.
 *   - param:
 *        Volume value to configure
 *
 *    example: Reference cli_usb.c
 *
 *   - Valid values needs to be viewed in camera(UVC+UAC).
 *
 *  Choose to call this API.
 *
 */
bk_err_t bk_usb_uac_set_volume(uint32_t param);

/**
 * @brief     Get UAC volume value
 *
 * This API Get volume:
 *   - One parameters param.
 *   - param:
 *        Pass in the pointer and get the value through the pointer.
 *
 *    example: Reference cli_usb.c
 *
 *   - Valid values needs to be viewed in audio.
 *
 *  Choose to call this API.
 *
 */
bk_err_t bk_usb_uac_get_volume(uint32_t *param);
